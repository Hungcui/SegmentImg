...
def downsample_block(x, n_filters, dropout=0.1, use_bn=True):
    f = double_conv_block(x, n_filters, use_bn=use_bn)
    p = layers.MaxPool2D(2)(f)
    p = layers.Dropout(dropout)(p)
    return f, p

def upsample_block(x, conv_feature, n_filters, dropout=0.1, use_bn=True):
...
def build_unet_with_boundary(input_shape=(512, 512, 3), num_classes=6, dropout=0.1, use_batchnorm=False):
...
class MultiRootVOCDataset:
    ...
    def __init__(self, roots: List[str], image_set: str,
                 ...
                 crop_size: int = 512, random_scale=(0.95, 1.05),
                 hflip_prob: float = 0.2, ignore_index: int = 255,
                 resize_prob: float = 0.3, center_crop_jitter: float = 0.05):
        super().__init__()
        ...
        self.center_crop_jitter = float(center_crop_jitter)
    ...
    def _random_resize(self, img, mask):
        """Lighter random resize: tight scale range, optional application."""
        if self.random_scale and np.random.rand() < self.resize_prob:
            lo, hi = self.random_scale
            # sanity clamp
            lo = max(0.5, float(lo)); hi = min(2.0, float(hi))
    ...
    def _random_crop(self, img, mask):
        """
        Center-biased crop with small jitter.
        - If image smaller than crop_size, pad as before.
        - Otherwise, pick crop around center with limited offset.
        """
        ...
        ci = max_i // 2
        cj = max_j // 2
        # jitter is a small fraction of the available margin
        jitter = self.center_crop_jitter
        di = int(round(max_i * jitter))
        dj = int(round(max_j * jitter))
        # sample offsets around center and clamp
        i = int(np.clip(ci + np.random.randint(-di, di + 1), 0, max_i))
        j = int(np.clip(cj + np.random.randint(-dj, dj + 1), 0, max_j))
    ...
    def get_item(self, idx):
        ...
        if self.image_set == "train":
            img, mask = self._random_resize(img, mask)    # lighter resize
            img, mask = self._random_crop(img, mask)      # center-biased crop
            img, mask = self._hflip(img, mask)            # lower prob by default (0.3)
        else:
